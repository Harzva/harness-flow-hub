...
const [pkg, client, bootstrapSource, patch, bootstrap, lifecycle, offline, flow] = await Promise.all([
  json('package.json'),
...
const report = {
  ...
  commands: ['pnpm check', 'pnpm run ui:verify-bootstrap-compatibility', 'pnpm run ui:verify-lifecycle', 'pnpm run ui:verify-registry-offline', 'pnpm run transaction:verify-flow', 'pnpm run m2:verify-core'],
  components: {
