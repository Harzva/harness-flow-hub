...
import { summarizeActionOutcome } from '../action-outcome.js'
...
function ActionResult({ result, openProfiles }: { result: ActionResponse, openProfiles: () => void }): ReactNode {
  const outcome = summarizeActionOutcome(result)
  return <section className={`flowHubResult flowHubResult--${outcome.state}`} role={result.ok ? 'status' : 'alert'} aria-label="操作结果"><h4>{outcome.title}</h4><dl><dt>发生了什么</dt><dd>{outcome.happened}</dd><dt>是否已回滚</dt><dd>{outcome.rollback}</dd><dt>下一步</dt><dd>{outcome.next}</dd></dl><div className="flowHubActions"><button className="flowHubButton" type="button" onClick={openProfiles}>查看 Profiles</button></div></section>
}
...
function FlowCatalog({ flows }: { flows: FlowCatalogEntry[] }): ReactNode {
  ...
  useEffect(() => {
    if (flow !== undefined && flow.id !== flowId) setFlowId(flow.id)
  }, [flow, flowId])
  useEffect(() => {
  ...
  return <div className="flowHubFlowLayout">
    <div className="flowHubFlowPicker" aria-label="选择 Harness Flow">{flows.map(item => <button key={item.id} type="button" aria-pressed={item.id === flow.id} onClick={() => { setFlowId(item.id); setVariantId(item.variants[0]?.id ?? 'lite') }}><span>{item.category}</span><b>{item.name}</b><small>{item.variants.length} 个变体</small></button>)}</div>
  ...
  return (
    <section className="flowHubShell">
      <style>{`
        ...
        .flowHubPanel{border:1px solid var(--fh-line);border-radius:13px;padding:18px;margin-top:12px;background:color-mix(in srgb,currentColor 2%,transparent)}.flowHubPanel h4{margin:0 0 8px;font-size:14px}.flowHubPanel p{margin:0;opacity:.64;font-size:12px;line-height:1.65}.flowHubActions{display:flex;flex-wrap:wrap;gap:8px;margin-top:15px}.flowHubButton{min-height:44px;border:1px solid var(--fh-line);border-radius:8px;padding:8px 13px;background:transparent;color:inherit;cursor:pointer;font-weight:650}.flowHubButton:hover:not(:disabled){border-color:var(--fh-accent);transform:translateY(-1px)}.flowHubButton--primary{background:var(--fh-accent);border-color:var(--fh-accent);color:#21170e}.flowHubButton:disabled{opacity:.38;cursor:not-allowed}.flowHubButton:focus-visible,.flowHubNav button:focus-visible,.flowHubPlugin:focus-visible,.flowHubSearch:focus-visible,.flowHubPlan:focus-visible,.flowHubVariantTabs button:focus-visible,.flowHubFlowPicker button:focus-visible,.flowHubChoice:focus-visible{outline:3px solid color-mix(in srgb,var(--fh-accent) 78%,white);outline-offset:2px}.flowHubPlan{margin-top:14px;border:1px solid color-mix(in srgb,var(--fh-accent) 45%,var(--fh-line));border-radius:11px;padding:14px}.flowHubPlanHint{margin:6px 0 0;font-size:11px;opacity:.66}.flowHubPlan dl{display:grid;grid-template-columns:78px 1fr;gap:7px;margin:10px 0;font-size:11px}.flowHubPlan dt{opacity:.5}.flowHubPlan dd{margin:0;overflow-wrap:anywhere}
        .flowHubChooser{margin-top:12px;border:1px solid var(--fh-line);border-radius:14px;padding:18px;background:linear-gradient(125deg,color-mix(in srgb,var(--fh-accent) 8%,transparent),transparent 62%)}.flowHubChooser h4{margin:0;font:600 18px/1.25 "Iowan Old Style","Noto Serif SC",serif}.flowHubChooser>p{margin:6px 0 0;font-size:12px;line-height:1.6;opacity:.65}.flowHubChoices{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:14px}.flowHubChoice{min-height:142px;border:1px solid var(--fh-line);border-radius:11px;padding:15px;background:color-mix(in srgb,currentColor 2%,transparent);color:inherit;text-align:left;cursor:pointer;display:flex;flex-direction:column;align-items:flex-start}.flowHubChoice:hover{border-color:color-mix(in srgb,var(--fh-accent) 60%,var(--fh-line));background:color-mix(in srgb,var(--fh-accent) 7%,transparent);transform:translateY(-1px)}.flowHubChoice small{font:700 9px/1.2 ui-monospace,monospace;letter-spacing:.11em;color:var(--fh-accent)}.flowHubChoice strong{margin-top:9px;font-size:15px}.flowHubChoice span{margin-top:6px;font-size:11px;line-height:1.55;opacity:.64}.flowHubChoice b{margin-top:auto;padding-top:12px;font-size:11px;color:var(--fh-accent)}.flowHubRule{margin:12px 0 0!important;border-left:3px solid var(--fh-accent);padding:8px 10px;background:color-mix(in srgb,var(--fh-accent) 7%,transparent);opacity:.86!important}
      ...
      <div className="flowHubBody">
        ...
        <main ref={mainRef} className="flowHubMain" tabIndex={-1} aria-busy={running !== null}>
          ...
          {view === 'home' ? <><div className="flowHubSectionHead"><div><h3>可信能力地图</h3><p>Registry 只陈述证据，不把“被发现”包装成“已可信”。</p></div><button className="flowHubButton" type="button" onClick={refresh}>刷新状态</button></div><section className="flowHubChooser" aria-labelledby="flow-hub-choose-title"><h4 id="flow-hub-choose-title">你现在需要哪一种？</h4><p>先按目标选择。这里不会立即安装；下一页仍会先展示来源、权限、风险和验证证据。</p><div className="flowHubChoices"><button className="flowHubChoice" type="button" onClick={() => { setView('plugins') }}><small>PLUGIN / 单项能力</small><strong>我只缺一个工具或连接</strong><span>给现有 Profile 增加一种能力，例如搜索、PDF、GitHub 或记忆；如何组合与运行由你决定。</span><b>浏览插件 →</b></button><button className="flowHubChoice" type="button" onClick={() => { setView('flows') }}><small>FLOW / 完整专家方案</small><strong>我想直接得到一套工作环境</strong><span>一次预览角色与边界、Skills、Plugins、Memory、Workflow、Permissions、默认配置和验收任务；安装时创建独立 Profile 与 Stack lock。</span><b>选择 Flow →</b></button></div><p className="flowHubRule"><b>一句话记住：</b>插件增加一项能力；Flow 交付一套可执行、可验证、可恢复的专家工作台。</p></section><div className="flowHubGrid"><Metric value={registry?.plugins.length ?? '—'} label="候选插件" note={`Registry ${registry?.registryVersion ?? '载入中'}`} /><Metric value={verifiedCount} label="验证通过" note="完整运行证据" /><Metric value={failedCount} label="验证失败" note="失败同样公开" /><Metric value={registry?.flows.length ?? 0} label="专家 Flow" note="即将进入首发批次" /></div><div className="flowHubPanel"><h4>测试安装通道</h4><p>当前 Alpha 只允许固定的 hello bundle 进入写操作。兼容性不确定时，Bootstrap 会自动保持只读。</p><div className="flowHubActions"><button className="flowHubButton flowHubButton--primary" disabled={blocked || profiles[0]?.plugin.installed === true} onClick={event => { prepare('add', event.currentTarget) }}>{running === 'add' ? '生成计划中…' : profiles[0]?.plugin.installed ? '已安装' : '安装测试 Bundle'}</button><button className="flowHubButton" disabled={blocked || profiles[0]?.plugin.installed !== true} onClick={event => { prepare('update', event.currentTarget) }}>{running === 'update' ? '生成计划中…' : '更新'}</button><button className="flowHubButton" disabled={blocked || profiles[0]?.plugin.installed !== true} onClick={event => { prepare('remove', event.currentTarget) }}>{running === 'remove' ? '生成计划中…' : '卸载'}</button></div>{plan ? <PlanPreview plan={plan} running={running} execute={execute} cancel={cancelPlan} /> : null}{result ? <ActionResult result={result} openProfiles={() => { setView('profiles') }} /> : null}</div></> : null}
          {view === 'plugins' ? <>
