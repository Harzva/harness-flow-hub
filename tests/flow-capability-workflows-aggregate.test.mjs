...
test('cross-platform Flow capability evidence keeps partial UI proof fail-closed', () => {
  ...
  for (const platform of Object.values(evidence.platforms)) {
    ...
    assert.equal(platform.research.package, 'dsh-science-workbench@0.1.1')
    assert.equal(platform.research.officialToolRuntimePipeline, true)
    assert.equal(platform.research.artifactHashesComplete, true)
    assert.equal(platform.research.feedbackLineageRecorded, true)
    assert.equal(platform.research.sourcePreserved, true)
    assert.equal(platform.research.networkCalled, false)
  ...
  assert.match(evidence.capabilityDecision.researchExpert, /science-workbench-fixture-workflow-passed-cross-platform/)
